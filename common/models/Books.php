...
class Books extends \yii\db\ActiveRecord
{
    ...
    public function createBook($bookData)
    {
        if(!Books::find()->where(['title' => $bookData['title']])->exists()) {
            ...
            $this->image = isset($bookData['thumbnailUrl']) ? $this->createImage($bookData['thumbnailUrl']) : 'missingPicture.jpg';
            $this->shortDescription = isset($bookData['shortDescription']) ? $bookData['shortDescription'] : $this->substituteDescription($bookData);
            $this->longDescription = isset($bookData['longDescription']) ? $bookData['longDescription'] : $this->substituteDescription($bookData);
    ...
    public function createImage($bookData)
    {
        ...
        $path = Yii ::getAlias('@common').'/images/books/';
    ...
    public function substituteDescription($bookData)
    {
        if (isset($bookData['longDescription'])) {
            return mb_substr($bookData['longDescription'], 0, 300) . '...';
        } else {
            return 'Книга очень интересная, но без текста((';
        }
    }
